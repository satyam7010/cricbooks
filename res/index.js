...
class Player {
    ...
    throwBall(event, extraRuns=0) {
        ...
        row[1].innerText = `${Math.floor(this.bowl.overs / 6)}.${this.bowl.overs % 6}` // Convert balls to overs
...
class Cricket {
    constructor(teamA, teamB) {
        this.teamA = {id: 'teamA', total: 0, wickets: 0, overs: 0, players: [], order: null}
        Array.from(teamA.children).forEach((player, ind) => {
            this.teamA.players.push(new Player(ind, player.id))
        })

        this.teamB = {id: 'teamB', total: 0, wickets: 0, overs: 0, players: [], order: null}
    ...
    async setStriker() {
        ...
        const players = this.battingTeam.players
            .filter(player => !player.bat.out && player.id != this.nonStriker?.id && player.id != this.striker?.id)
        ...
        if(players.length) {
            ...
            this.striker = this.battingTeam.players.filter(player => player.name == striker)[0]
        ...
        // No one else left to bat, that is, only non-striker is left
            ...
            document.querySelector(`#${this.nonStriker.name}-bat`).classList.remove('border-warning')
            this.nonStriker = null
        }
    }
    ...
    async setNonStriker() {
        ...
        if(players.length) {
            ...
            this.nonStriker = this.battingTeam.players.filter(player => player.name == nonStriker)[0]
    ...
    async setBowler() {
        ...
        const players = this.bowlingTeam.players.filter(player => player.id != this.bowler?.id).map(player => player.name)
        ...
        if(players.length) {
            ...
            this.bowler = this.bowlingTeam.players.filter(player => player.name == bowler)[0]
    ...
    async handleEvent(event) {
        ...
        // In case of wicket
            ...
            if(this.nonStriker) {
                const wicket = await getPrompt('Who got Out?', [this.striker.name, this.nonStriker.name])
            ...
            // If it was one-man batting, then batting team is all out
                ...
                // If it was chasing team, find match draw or lost
                    ...
                    if(this.battingTeam.total == this.bowlingTeam.total) { // Draw
                        outcome = [null, null, 'Draw']
                    } else { // Batting team Lost
        ...
        // In case runs was scored
            ...
            if(event % 2 == 1) { // If single or three, rotate strike
                this.rotate()
            }
        ...
        if(this.battingTeam.overs == this.totalOvers * 6) {
            ...
            // If it was the chasing team, find match outcome
                ...
                if(this.battingTeam.total == this.bowlingTeam.total) { // Draw
                    outcome = [null, null, 'Draw']
                } else if(this.battingTeam.total < this.bowlingTeam.total) { // Batting team Lost
                    let margin = this.bowlingTeam.total - this.battingTeam.total
                    outcome = [this.bowlingTeam, margin, 'Runs']
                } else { // Batting team won
                    let margin = this.battingTeam.players.length - this.battingTeam.wickets
                    outcome = [this.battingTeam, margin, 'Wickets']
                }
            }

        // If over completed, then change bowler and rotate strike
        } else if(this.battingTeam.overs > 0 && this.battingTeam.overs % 6 == 0 && !outcome) {
            await this.setBowler()
            this.rotate()
        }
    ...
    async updateMatchStatus(team, margin, status) {
        ...
        // End game
            ...
            if(status == 'Draw') {
                ...
                msg = `${team.id} WON by ${margin} ${status}`
            }
...
function generateScoreBoard() {
    const tabs = `<div class="pt-2">
        <ul class="nav nav-tabs nav-justified mb-3" id="scoreTab" role="tablist">
            <li class="nav-item" role="presentation">
                <a class="nav-link active fw-bolder" id="${match.battingTeam.id}-tab" data-bs-toggle="tab" href="#${match.battingTeam.id}-content" role="tab" aria-controls="${match.battingTeam.id}-content" aria-selected="true">${match.battingTeam.id}</a>
            </li>
            <li class="nav-item" role="presentation">
                <a class="nav-link fw-bolder" id="${match.bowlingTeam.id}-tab" data-bs-toggle="tab" href="#${match.bowlingTeam.id}-content" role="tab" aria-controls="${match.bowlingTeam.id}-content" aria-selected="true">${match.bowlingTeam.id}</a>
            </li>
        </ul>

        <div class="tab-content" id="scoreTabContent">
            <div class="tab-pane fade show active" id="${match.battingTeam.id}-content" role="tabpanel" aria-labelledby="${match.battingTeam.id}-tab">
            ${createTab(match.battingTeam, match.bowlingTeam)}
            </div>
            <div class="tab-pane fade" id="${match.bowlingTeam.id}-content" role="tabpanel" aria-labelledby="${match.bowlingTeam.id}-tab">
    ...
    return new Promise((resolve, reject) => {
        ...
        confirm.addEventListener('click', () => {
            ...
            if(typeof(options) != 'string') {
                ...
                const selectedPlayer = document.querySelector('input[name="options"]:checked')
                if(selectedPlayer) {
